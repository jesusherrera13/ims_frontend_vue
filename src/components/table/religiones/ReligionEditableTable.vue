...
onMounted(async () => {
    store.fetchReligiones();
});

const getReligiones: any = computed(() => {
    return store.religiones;
    /* return store.religiones.filter((religion: any) => {
        return (religion.id + ' ' + religion.nombre).toLowerCase().includes(search.value.toLowerCase());
    }); */
});
...
const headers: any = ref([
    { title: 'Religión', align: 'start', key: 'nombre' },
...
async function confirmDelete() {
    try {
        Object.assign(store.religion, itemToDelete.value);
        const response = store.delete();

        response.then(() => {
            store.fetchReligiones();
            showAlert('success', 'Religión eliminado con éxito');
        }).catch(error => {
            showAlert('error', 'Error al eliminar la Religión');
        });
...
function save() {
    Object.assign(store.religion, editedItem.value);
    let response;
    if (store.religion.id) {
        response = store.update();
        showAlert('success', 'Religión actualizada con éxito');
    } else {
        response = store.store();
        showAlert('success', 'Religión guardada con éxito');
    }

    response.then(() => {
        store.fetchReligiones();
    }).catch(error => {
        showAlert('error', 'Error al guardar la Religión');
    });
...
const formTitle = computed(() => {
    return editedIndex.value === -1 ? 'Nueva Religión' : 'Editar Religión';
});
...
<template>
    ...
    <v-dialog v-model="deleteDialog" persistent max-width="500">
        <v-card>
            ...
            <v-card-text>
                ¿Estás seguro de que deseas eliminar esta religión?
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
            ...
            <v-dialog v-model="dialog" max-width="500" persistent>
                <template v-slot:activator="{ props }">
                    <v-btn color="primary" v-bind="props" flat class="ml-auto">
                        <v-icon class="mr-2">mdi-account-multiple-plus</v-icon>Nueva Religión
                    </v-btn>
                </template>
                <v-card>
                    ...
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            <v-row>
                                <v-col cols="12">
                                    <v-text-field variant="outlined" hide-details v-model="editedItem.nombre" label="Religión"></v-text-field>
                                </v-col>
                            </v-row>
                        </v-form>
                    </v-card-text>
