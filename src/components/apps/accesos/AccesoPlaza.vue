...
const getEmpresaPlaza: any = computed(() => {
    // se obtienen las empresas y plazas
...
function handleClick(e: any, row: any) {
    ...
    for (var i = 0; i < inputElements.length; i++) {
        // se recorre el grupo de elementos y se les asigna el valor del checkbox
    ...
    for (var i = 0; i < inputElements.length; i++) {
        // se recorre el grupo de elementos
...
watch([() => store.user.id], () => {
    // se observa el cambio del usuario
...
watch([() => storeAccesoPlaza.access_user_modules], () => {
    ...
    // se observa el cambio de los modulos del usuario
    let tmp = storeAccesoPlaza.access_user_modules.map((item: any) => {
        // se obtienen los modulos del usuario
    ...
    for (var i = 0; i < inputElements.length; i++) {
        ...
        for (var j in tmp) {
            // se recorre el arreglo de modulos
